-- Alinea la firma consumida por el API con la creación de arreglos que puede
-- registrar un cobro inicial. Se reemplaza la firma anterior para evitar que
-- PostgREST tenga overloads ambiguos en su schema cache.
DROP FUNCTION IF EXISTS public.rpc_crear_arreglo_completo(
  uuid, uuid, public.estado_arreglo, text, integer, timestamptz, text,
  numeric, numeric, boolean, jsonb, jsonb, jsonb, jsonb, jsonb
);
