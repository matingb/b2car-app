-- La creación de arreglos se hace a través de esta RPC. La marca fiscal se
-- resuelve con el claim firmado para que BASE nunca persista un arreglo
-- facturable, incluso fuera de la UI.
...
BEGIN
  ...
  SET combustible_leido = p_combustible_leido,
      ...
      es_facturable = CASE
        WHEN (SELECT auth.jwt() ->> 'plan_sub') = 'PRO' THEN COALESCE(p_es_facturable, false)
        ELSE false
      END,
      updated_at = now()
